...
export const MapViewer: React.FC = () => {
  ...
  const { isCapturing, currentCaptureDataUrl } = useMapCapture(mapRef);
  ...
  const setDetectionResult = useDetectionStore(
    (state) => state.setDetectionResult,
  );
  const setDetectionError = useDetectionStore(
    (state) => state.setDetectionError,
  );
  ...
  useEffect(() => {
    ...
    if (!apiKey || apiKey === "YOUR_DEMO_KEY") {
      setMapError(
        "Google Maps API key not configured. Please set VITE_GOOGLE_MAPS_API_KEY in .env.local",
      );
      return;
    }
  ...
  const handleCaptureAndDetect = useCallback(async () => {
    ...
    try {
      ...
      // let width = 640;
      // let height = 640;
      const width = Math.min(Math.floor(rect.width), 2048);
      const height = Math.min(Math.floor(rect.height), 2048);
      ...
      reader.onloadend = async () => {
        ...
        try {
          const result = await DetectionService.detectFromBase64(
            base64data,
            selectedModel,
          );
  ...
  return (
    <div className="map-viewer">
      ...
      {detecting && (
        <LoadingSpinner message="Running detection on map view..." />
      )}
    </div>
  );
};
